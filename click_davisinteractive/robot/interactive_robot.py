...
class InteractiveScribblesRobot(object):
    ...
    def _mask2graph(self, skeleton_mask):
        ...
        X, Y = X.ravel(), Y.ravel() #-> np.array (size,)
        M = mask.ravel()            #-> np.array (size,)
    ...
    def interact(self,
        ...
        scribbles = [[] for _ in range(nb_frames)]  # init. empty scribbles
        ...
        for obj_id in obj_ids:
            ...
            logging.verbose(
                'Time to compute the skeleton mask: {:.3f} ms'.format(
                    skel_time * 1000), 2)
            if skel_mask.sum() == 0:
                print("Well skel_mask sum is equal to 0")
                continue
