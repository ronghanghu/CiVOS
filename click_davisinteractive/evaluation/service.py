...
class EvaluationService:
    ...
    def get_gt_mask(self, sequence):    # Vujas
        gt_data = self.davis.load_annotations(sequence)
    ...
    def post_predicted_masks(self,
        ...
        prev_frames_used = self.storage.get_annotated_frames(
            session_key, sequence, scribble_idx)        # class'tuple':()
        prev_frames_used = set(prev_frames_used)        # class'set':()

        # next_scribble_frame_candidates is None in my case vujas
        override = next_scribble_frame_candidates is not None   # False
        next_scribble_frame_candidates = next_scribble_frame_candidates or [
            i for i in range(nb_frames) # All frames where an annotations can be performed, list
        ]
        next_scribble_frame_candidates = set(next_scribble_frame_candidates) # All frames where an annotations can be performed, set()
        ...
        metric_idx = metric.argsort() # Sort through arguments
        i = 0
    ...
    def summarize_report_vujas(self, df, round_results = None, use_time_axis= True):
        ...
        # Arguments
            ...
            round: decimal to round the results
            use_time_axis: Use time axis or interaction ?
        ...
        if round_results is not None:
            auc       = np.round(auc,round_results)
            metric_th = np.round(metric_th,round_results)
